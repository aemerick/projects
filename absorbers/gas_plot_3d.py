...
class GalCluster:
    def __init__(self, pf, center, R_vir, inputfile=None):
        self.pf = pf
        self.center= center
    ...
    def plot_points(self, cparam=None):
        ...
        for a in self.absorbers:
            ...
            if a.plot:
                ...
                xpos.append(pos[0]*conv)
                ypos.append(pos[1]*conv)
                zpos.append(pos[2]*conv)
...
#    absorber_index = np.where( ray_N == np.max(ray_N[nearest]) )
    ...
    vel = np.array([ ray['x-velocity'][absorber_index],
                     ray['y-velocity'][absorber_index],
                     ray['z-velocity'][absorber_index]])
    ...
    for field in additional_fields:
        params[field] = ray[field][absorber_index]
   ...
   # params["HI_Density"] = ray["HI_Density"][absorber_index][0]
    ...
    params['vpec'] = vlos[absorber_index]
...
def find_absorber_index(ray_N, abs_N, ray_v, abs_v, method='old'):
    ...
    #    ray_N, abs_N = np.log10(ray_N), np.log10(abs_N)
      ...
      #  ray_N, abs_N = np.log10(ray_N), np.log10(abs_N)
        ...
        outside_range = np.abs(shifted_v) >= vcut
    
        ray_N[outside_range] = 1.0E50 # shift ray_N to silly values
...
def plot_gas_3D(cluster,outdir,data, draw_velocity=True, pec_vel=True):
   ...
   # vmin, vmax = np.log10(vmin), np.log10(vmax)
    ...
    for i in np.arange(num_abs):
        ...
        pos, params = find_absorber_position_velocity(pf, ray, cz, N, pec_vel=pec_vel)
